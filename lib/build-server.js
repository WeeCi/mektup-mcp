...
export function createMektupServer(apiKey, apiBase = "https://api.usemektup.com") {
  ...
  tool("get_mailbox_webhook", "Check a mailbox's configured webhook URL (never returns the signing secret - only shown once, at setup or rotation).",
    { domain: domainDesc, localPart: localPartDesc },
    ({ domain, localPart }) => call("GET", `/v1/domains/${encodeURIComponent(domain)}/mailboxes/${encodeURIComponent(localPart)}/webhook`));

  tool("set_mailbox_webhook",
    "Set or update the URL that fires on every new inbound message to this mailbox - the way an agent finds out about new mail without polling list_messages. Must be https. Returns a signing secret the FIRST time it's set (or if regenerateSecret is passed) - shown once, not recoverable later. Every delivery is a POST with an X-Mektup-Signature: sha256=<hex> header, an HMAC-SHA256 of the raw JSON body using that secret - verify it before trusting a payload.",
    {
      domain: domainDesc,
      localPart: localPartDesc,
      url: z.string().url().describe("https:// URL to POST new-mail events to"),
      regenerateSecret: z.boolean().optional().describe("Issue a new signing secret even if one already exists - invalidates the old one"),
    },
    ({ domain, localPart, url, regenerateSecret }) =>
      call("PUT", `/v1/domains/${encodeURIComponent(domain)}/mailboxes/${encodeURIComponent(localPart)}/webhook`, { url, regenerateSecret }));

  tool("delete_mailbox_webhook", "Remove a mailbox's webhook - new mail stops firing it immediately.",
    { domain: domainDesc, localPart: localPartDesc },
    ({ domain, localPart }) => call("DELETE", `/v1/domains/${encodeURIComponent(domain)}/mailboxes/${encodeURIComponent(localPart)}/webhook`));
